"""Module tree builder for grouping graph nodes by module path.
...
from sqlmodel_nexus.voyager.type import ModuleNode, ModuleRoute, Route, SchemaNode

N = TypeVar('N')  # Node type: ModuleNode or ModuleRoute
_ItemType = TypeVar('_ItemType')  # Item type: SchemaNode or Route


def _build_module_tree(
    items: list[_ItemType],
    *,
    get_module_path: Callable[[_ItemType], str | None],
    NodeClass: type[N],
    item_list_attr: str,
) -> list[N]:
    """Generic builder that groups items by dotted module path into a tree of NodeClass."""
    top_modules: dict[str, N] = {}
    root_level_items: list[_ItemType] = []
    ...
    def get_or_create(child_name: str, parent: N) -> N:
        ...
        if not parent_full or parent_full == "__root__":
            fullname = child_name
        else:
            fullname = f"{parent_full}.{child_name}"
